...
async def get_pipeline_last_run(db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Common in unit tests using SQLite where pipeline_runs table isn't created.
        # Treat as "no pipeline runs" instead of 500.
        msg = str(e).lower()
        if "no such table" in msg and "pipeline_runs" in msg:
            raise HTTPException(
                status_code=404,
                detail="Pipeline runs table not found. Run the batch ETL pipeline first."
            )
        logger.error(f"Database operational error fetching pipeline last run: {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail="Database error fetching pipeline status.")
    except Exception as e:

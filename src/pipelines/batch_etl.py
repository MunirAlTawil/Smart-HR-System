...
def extract_description(poi: Dict[str, Any]) -> Optional[str]:
    ...
    Handles hasDescription as list of objects with shortDescription.{fr,en},
    or description/hasDescription as dict/string.
    """
    desc_obj = poi.get("hasDescription") or poi.get("description")

    # hasDescription is often a list: [{"shortDescription": {"fr": "..."}}]
    if isinstance(desc_obj, list) and len(desc_obj) > 0:
        first = desc_obj[0]
        if isinstance(first, dict):
            short_desc = first.get("shortDescription")
            ...
            # no shortDescription: try direct fr/en on first element
            desc = first.get("fr") or first.get("@fr") or first.get("en") or first.get("@en")
            if desc:
                return str(desc).strip()

...
def fetch_updates_chart(days: int = 90) -> tuple[Optional[List[Dict[str, Any]]], Optional[str]]:
    """Fetch day counts from /charts/updates endpoint.

    Returns:
        (data, error): On success, data is the list (possibly empty); error is None.
        On request failure, data is None and error is the message to show.
    """
    try:
        response = requests.get(f"{API_BASE_URL}/charts/updates", params={"days": days}, timeout=5)
        response.raise_for_status()
        data = response.json()
        # Success: return data (may be empty list) and no error
        return (data if isinstance(data, list) else [], None)
    except requests.exceptions.ConnectionError as e:
        return (None, f"Cannot connect to API at {API_BASE_URL}. Ensure the API server is running.")
    except requests.exceptions.Timeout as e:
        return (None, f"Request timed out while fetching update data from {API_BASE_URL}.")
    except requests.exceptions.HTTPError as e:
        msg = e.response.text if e.response is not None else str(e)
        return (None, f"HTTP error from API: {msg}")
    except Exception as e:
        logger.error(f"Error fetching updates chart: {str(e)}")
        return (None, f"Error fetching update data: {str(e)}")
...
# Updates Chart Page
    ...
    # Default 90 days so the API is more likely to return data (dataset may have no recent updates).
    days = st.sidebar.slider(
        "Number of days",
        min_value=7,
        max_value=90,
        value=90,
        help="Time range to aggregate POI updates by day."
    )
    st.sidebar.caption(
        "Smaller time ranges may return no results if there are no updates in that period. Try increasing the number of days."
    )

    with st.spinner("Loading update counts..."):
        update_data, fetch_error = fetch_updates_chart(days=days)

    # Request failure (e.g. connection, timeout, HTTP): show real error; do not confuse with empty data.
    if fetch_error is not None:
        st.error(f"❌ {fetch_error}")
        st.stop()

    # Success but empty list: API responded; no updates in the selected time range.
    if not update_data:
        st.warning(
            "No update data found for the selected time range. Try increasing the number of days."
        )
        st.stop()

    # We have data: render chart and table unchanged.

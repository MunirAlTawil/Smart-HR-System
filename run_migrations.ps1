...
# Migration 004: Add theme column (idempotent)
Write-Host "Running 004_add_theme_column.sql..." -ForegroundColor Yellow
Get-Content sql/migrations/004_add_theme_column.sql | docker compose exec -T postgres psql -U holiday -d holiday

# Migration 02: Consolidated idempotent migration (also runs automatically on API startup)
